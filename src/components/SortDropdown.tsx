  ...
  return (
    <div className="relative" ref={dropdownRef}>
      <button
        ...
        className="flex items-center justify-center gap-2 px-5 py-2.5 sm:px-5 sm:py-2.5 px-3 bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-300 rounded-lg font-medium border border-slate-200 dark:border-slate-600 hover:bg-slate-100 dark:hover:bg-slate-700 transition-all duration-200 sm:min-w-[180px] min-w-[44px] min-h-[44px]"
      >
        <ListFilter className="w-5 h-5 sm:hidden" />
        <span className="text-sm hidden sm:inline">Sort by: {selectedOption.label}</span>
        <ChevronDown
          className={`w-4 h-4 hidden sm:inline transition-transform duration-200 ${isOpen ? 'rotate-180' : ''}`}
        />
      </button>
